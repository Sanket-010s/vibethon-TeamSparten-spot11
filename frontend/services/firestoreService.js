...
import { HEATMAP_DAYS, ML_MODULES } from "@/constants/data";

const STORAGE_KEY = "aiml_nexus_demo_user_data";

export const BADGE_DEFINITIONS = [
  { id: "starter", label: "Starter", description: "Complete your first module." },
  { id: "quiz-sprinter", label: "Quiz Sprinter", description: "Finish a quiz attempt." },
  { id: "streak-3", label: "3-Day Streak", description: "Learn on three days in a row." },
  { id: "builder", label: "Builder", description: "Reach 100 total points." },
  { id: "mentor", label: "Mentor", description: "Reach 300 total points." },
  { id: "explorer", label: "Explorer", description: "Complete every learning module." },
];

const DEMO_LEADERBOARD = [
  { uid: "demo-1", name: "Aarav Mehta", totalPoints: 320, currentStreak: 9, longestStreak: 14, badgeIds: ["starter", "quiz-sprinter", "streak-3", "builder", "explorer"] },
  { uid: "demo-2", name: "Nisha Rao", totalPoints: 286, currentStreak: 6, longestStreak: 11, badgeIds: ["starter", "quiz-sprinter", "streak-3", "builder"] },
  { uid: "demo-3", name: "Kabir Shah", totalPoints: 240, currentStreak: 4, longestStreak: 8, badgeIds: ["starter", "quiz-sprinter", "streak-3"] },
  { uid: "demo-4", name: "Mira Patil", totalPoints: 195, currentStreak: 5, longestStreak: 7, badgeIds: ["starter", "quiz-sprinter"] },
];
...
function applyRewardState(data = {}) {
  ...
  return { ...normalized, currentStreak, longestStreak, badgeIds, progressPercent, rewardPoints: normalized.totalPoints || 0 };
}
...
export async function ensureUserDocument(uid, payload = {}) {
  if (isFirebaseConfigured) {
    ...
    if (!snap.exists()) {
      await setDoc(ref, { ...defaultUserData(payload.skillLevel), email: payload.email || "", createdAt: serverTimestamp() });
    } else {
...
export function subscribeToUserDocument(uid, callback) {
  if (isFirebaseConfigured) {
    ...
    return onSnapshot(ref, (snap) => {
      callback(snap.exists() ? applyRewardState(snap.data()) : defaultUserData());
    });
  }
...
export async function completeModule(uid, moduleId) {
  ...
  if (isFirebaseConfigured) {
    const ref = doc(db, "users", uid);
    await updateDoc(ref, { completedModules: arrayUnion(moduleId), totalPoints: increment(25), [`activityMap.${day}`]: increment(1), updatedAt: serverTimestamp() });
    const snap = await getDoc(ref);
    await syncProgressState(ref, snap.exists() ? snap.data() : defaultUserData());
    return;
  }
  const local = readLocalData();
  if (!local.completedModules.includes(moduleId)) local.completedModules.push(moduleId);
...
export async function saveQuizAttempt(uid, moduleId, scorePercent) {
  ...
  if (isFirebaseConfigured) {
    const ref = doc(db, "users", uid);
    await updateDoc(ref, { [`quizScores.${moduleId}`]: scorePercent, quizAttempts: arrayUnion({ moduleId, scorePercent, at: new Date().toISOString() }), totalPoints: increment(15), [`activityMap.${day}`]: increment(1), updatedAt: serverTimestamp() });
    const snap = await getDoc(ref);
    await syncProgressState(ref, snap.exists() ? snap.data() : defaultUserData());
    return;
  }
...
export async function saveGameScore(uid, gameId, score) {
  ...
  if (isFirebaseConfigured) {
    const ref = doc(db, "users", uid);
    await updateDoc(ref, { [`gameScores.${gameId}`]: score, totalPoints: increment(10), [`activityMap.${day}`]: increment(1), updatedAt: serverTimestamp() });
    const snap = await getDoc(ref);
    await syncProgressState(ref, snap.exists() ? snap.data() : defaultUserData());
    return;
  }
...
export async function fetchLeaderboardEntries(count = 8) {
  if (isFirebaseConfigured) {
    ...
    return snap.docs.map((item, index) => {
      ...
      return { uid: item.id, rank: index + 1, name: data.displayName || data.email || `Learner ${index + 1}`, ...data };
    });
  }
  return DEMO_LEADERBOARD.slice(0, count).map((entry, index) => ({ ...entry, rank: index + 1 }));
}
