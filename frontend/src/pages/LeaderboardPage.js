...
import { BADGE_DEFINITIONS, fetchLeaderboardEntries } from '../services/firestoreService';
...
export default function LeaderboardPage() {
  ...
  const badgeLookup = useMemo(() => Object.fromEntries(BADGE_DEFINITIONS.map((badge) => [badge.id, badge])), []);
  ...
  const mergedEntries = useMemo(() => {
    ...
    return nextEntries.sort((left, right) => (right.totalPoints || 0) - (left.totalPoints || 0)).slice(0, 8).map((entry, index) => ({ ...entry, rank: index + 1 }));
  }, [entries, progress, user]);

  if (loading || entriesLoading) return <LoadingSpinner label="Loading leaderboard" />;
    ...
    <section className={styles.wrap}>
      <header className={styles.header}>
        <div>
          ...
          <p className="page-subtitle">See who is leading on points, streaks, and badge unlocks.</p>
        </div>
      ...
      <div className={styles.podiumGrid}>
        {topThree.map((entry) => (
          <motion.article key={entry.uid} className={`${styles.podiumCard} ${entry.isCurrentUser ? styles.currentUser : ''}`} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <div className={styles.rankBadge}>#{entry.rank}</div>
            <h3>{entry.name}</h3>
            <p>{entry.totalPoints} points</p>
            <div className={styles.podiumStats}>
              <span><Flame size={14} /> {entry.currentStreak || 0} day streak</span>
              <span><BadgeCheck size={14} /> {(entry.badgeIds || []).length} badges</span>
            </div>
          </motion.article>
        ))}
      </div>
          ...
          {mergedEntries.map((entry) => (
            <div key={entry.uid} className={`${styles.row} ${entry.isCurrentUser ? styles.currentUserRow : ''}`}>
              ...
              <div className={styles.userMeta}>
                ...
                <span>{entry.currentStreak || 0} day streak · {entry.longestStreak || 0} best</span>
              </div>
