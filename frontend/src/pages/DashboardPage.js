...
import { TrendingUp, BookOpen, Award, Flame, Trophy } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useUserProgress } from '../hooks/useUserProgress';
import { BADGE_DEFINITIONS, buildHeatmapDays } from '../services/firestoreService';
import { ML_MODULES } from '../constants/data';
  ...
  return (
    <section className={styles.gridWrap}>
      ...
      <div className={styles.statsGrid}>
        {[
          { label: 'Progress', value: `${progress.progressPercent || 0}%`, icon: TrendingUp },
          { label: 'Modules Completed', value: `${completed.length}/${ML_MODULES.length}`, icon: BookOpen },
          { label: 'Total Points', value: progress.totalPoints || 0, icon: Award },
          { label: 'Current Streak', value: `${progress.currentStreak || 0} days`, icon: Flame },
          { label: 'Unlocked Badges', value: `${badgeIds.length}`, icon: Trophy },
        ].map((item, idx) => {
      ...
      <article className={`card ${styles.rewardCard}`}>
        <div className={styles.rewardHeader}>
          <div>
            ...
            <p>Keep learning daily to extend your streak and unlock more badges.</p>
          </div>
          <div className={styles.rewardNumbers}>
            <span><strong>{progress.currentStreak || 0}</strong> current streak</span>
            <span><strong>{progress.longestStreak || 0}</strong> best streak</span>
            <span><strong>{progress.rewardPoints || progress.totalPoints || 0}</strong> reward points</span>
          </div>
        </div>
        ...
        <div className={styles.chartArea}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData}>
              ...
              <Tooltip contentStyle={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)' }} />
              <Bar dataKey="score" fill="url(#colorGradient)" radius={[8, 8, 0, 0]} />
              <defs>
                <linearGradient id="colorGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="#ff6b35" stopOpacity={1} />
                  <stop offset="100%" stopColor="#ff8c5a" stopOpacity={0.8} />
                </linearGradient>
              </defs>
            </BarChart>
          </ResponsiveContainer>
        ...
        <div className={styles.chartArea}>
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendData.length ? trendData : [{ name: 'A1', score: 0 }]}>
              ...
              <YAxis domain={[0, 100]} stroke="var(--text-dim)" />
              <Tooltip contentStyle={{ background: 'var(--card-bg)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text)' }} />
              <Line type="monotone" dataKey="score" stroke="#ff6b35" strokeWidth={3} dot={{ fill: '#ff6b35', strokeWidth: 2, r: 5 }} activeDot={{ r: 8, fill: '#ff6b35', stroke: '#00d9ff', strokeWidth: 2 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </article>
