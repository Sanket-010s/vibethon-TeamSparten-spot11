    ...
    <section className={styles.wrap}>
        ...
        <div className={styles.dragGrid}>
          <div className={styles.items}>
            {GAME_ITEMS.classify.map((item) => (
              <button key={item.id} draggable onDragStart={(ev) => ev.dataTransfer.setData('text/plain', item.id)} className={styles.draggable} type="button">
                {item.label}
              </button>
            ))}
          </div>
          {['animals', 'vehicles'].map((bucket) => (
            <div key={bucket} className={styles.bucket} onDragOver={(ev) => ev.preventDefault()} onDrop={(ev) => onDrop(bucket, ev)}>
      ...
      <article className={`card ${styles.card}`}>
        ...
        <div className={styles.optionRow}>
          {[['apple', 'Apple'], ['carrot', 'Carrot'], ['bike', 'Bike']].map(([value, label]) => (
            <label key={value}>
      ...
      <div className={styles.submitRow}>
        <button className="button-primary" type="button" onClick={submitScores}>Submit Game Scores</button>
