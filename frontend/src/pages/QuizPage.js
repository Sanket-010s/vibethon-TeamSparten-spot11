...
import { saveQuizAttempt } from '../services/firestoreService';
import { trackEvent } from '../services/analytics';
import { ML_MODULES, QUIZ_BANK } from '../constants/data';
  ...
  return (
    <section className={styles.wrap}>
      ...
      <article className={`card ${styles.card}`}>
            ...
            <div key={q.id} className={styles.questionBlock}>
              ...
              <div className={styles.optionCol}>
                {q.options.map((opt, optIndex) => (
                  <label key={opt}>
                    <input type="radio" name={q.id} value={optIndex} checked={String(answers[q.id]) === String(optIndex)} onChange={(e) => setAnswers((prev) => ({ ...prev, [q.id]: e.target.value }))} />
                    {opt}
                  </label>
                ))}
              </div>
        ...
        <button className="button-primary" type="button" onClick={submitQuiz}>Submit Quiz</button>

        {result && (
          <div className={styles.result}>
            <strong>Score: {result.scorePercent}% ({result.correct}/{result.total})</strong>
